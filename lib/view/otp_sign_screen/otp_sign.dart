...
class OtpScreensign extends StatefulWidget {
  const OtpScreensign({super.key});

  @override
  State<OtpScreensign> createState() => _OtpScreenState();
}

class _OtpScreenState extends State<OtpScreensign> {
  String? ph;
  ...
  Future<void> _loadPhoneNumber() async {
    ...
    final savedPhone = pref.getString("Phone_s_key");
    if (mounted) {
      setState(() {
        ph = savedPhone;
      });
  ...
  void _verifyOtp() {
    ...
    if (otp.length == 6) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => const CreateNamePage()),
      );
    }
  }
